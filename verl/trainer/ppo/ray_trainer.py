...
class RayPPOTrainer:
    ...
    def _dump_generations(self, inputs, outputs, scores, reward_extra_infos_dict, dump_path,env_info,images=None):
        """Dump rollout/validation samples as JSONL."""
        os.makedirs(dump_path, exist_ok=True)
        filename = os.path.join(dump_path, f"global_step_{self.global_steps}.jsonl")
        ...
        with open(filename, "w") as f:
            for i in range(n):
                ...
                entry.update({
                    "env_idx": env_info[i][0],
                    "step_idx": env_info[i][1],
                })
                f.write(json.dumps(entry, ensure_ascii=False) + "\n")

        print(f"Dumped generations to {filename}")
        
        # 处理images参数，如果不为None则保存为PIL格式的图像
        if images is not None:
            ...
            # 创建保存图像的目录
            image_dir = os.path.join(dump_path, f"images_global_step_{self.global_steps}")
            os.makedirs(image_dir, exist_ok=True)
            print(f"Saving the images at global step {self.global_steps}")
            ...
            if not isinstance(images,np.ndarray):
                try:
                    ...
                    print(f"Warning: images format not supported. Expected numpy array, got {type(images)}")
                    print(f"Warning: try to convert images to numpy array but failed. Error: {e}")
            # 获取环境的数量（第一个维度的大小）
            num_envs = images.shape[0]
            # (3200,96,96,3)
            # print(f"images shape:{images.shape}")
            assert num_envs == len(env_info), f"Number of environments ({num_envs}) does not match number of env_info ({len(env_info)})"    
            # print(f"Found {num_envs} environments with images")
            
            # 遍历所有环境 
            for idx,(env_idx,env_step) in enumerate(env_info):
                # 获取当前环境的图像，只存前10个环境的交互情况，不然太多了会很慢
                if env_idx <= 10:
                    env_images = images[idx]  # 形状变为(96, 96, 3)
                    # print(f"idx:{idx}, env_idx:{env_idx}, env_step:{env_step}, env_images shape:",env_images.shape)
                    
                    # 确保图像数据在有效范围内（0-255）
                    if env_images.max() <= 1.0:
                        env_images = (env_images * 255).astype(np.uint8)
                    else:
                        env_images = env_images.astype(np.uint8)
                    
                    # 保存图像
                    img = Image.fromarray(env_images)
                    img.save(os.path.join(image_dir, f"env_{env_idx}_step_{env_step}.png"))
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            env_info = test_output_gen_batch.non_tensor_batch.get("env_info", [])
            if env_info is not None and len(env_info) > 0:
                env_info_list.extend(env_info)
            obs_image = test_output_gen_batch.non_tensor_batch.get("obs_image", [])
            # print(f'obs_image shape: {obs_image.shape}')
            if obs_image is not None and len(obs_image) > 0:
                images_list.extend(obs_image)
            batch_inputs = self.tokenizer.batch_decode(test_output_gen_batch.batch["prompts"], skip_special_tokens=True)
            sample_inputs.extend(batch_inputs)
        ...
        if validation_data_dir:
            ...
            images_list = None if len(images_list) == 0 else images_list
            # print("images_list len:",len(images_list))
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("gen", timing_raw):
                        ...
                        ################ agent-environment loop ###############
                        self.envs.set_training_step(self.global_steps)
                    ...
                    if rollout_data_dir:
                        with _timer("dump_rollout_generations", timing_raw):
                            ...
                            self._dump_generations(
                                ...
                                env_info=batch.non_tensor_batch.get("env_info", None),
                                images=batch.non_tensor_batch.get("obs_image", None),
                            )
