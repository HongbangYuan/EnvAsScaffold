    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                try:
                    current_gold_path = env.get_gold_action_sequence()
                    info['gold_actions'] = current_gold_path
                except Exception as e:
                    info['gold_actions'] = None
                try:
                    env.get_goal_progress()
                    info['goal_progress'] = env.get_goal_progress()
                except Exception as e:
                    info['goal_progress'] = None
                ...
                if data is None:
                    ...
                    task_id, task_variation = data
                    taskName = taskNames[task_id]
                simplification_str = simplifications_preset if simplifications_preset else ""
                env.load(taskName, task_variation, simplification_str, generateGoldPath=True)
                observation, info = env.reset()

                try:
                    gold_actions = env.get_gold_action_sequence()
                    info['gold_actions'] = gold_actions
                    logging.info(f"[DEBUG Worker] Raw Gold Actions from Java: {gold_actions}")
                except Exception as e:
                    info['gold_actions'] = None
